...
import type { MercuryRecipient } from '@mr.dj2u/mercury';
...
type MercuryRecipientManagerProps = {
  ...
  adapter: Pick<MercuryUiAdapter, 'createRecipient' | 'updateRecipient'>;
  busy?: boolean;
...
export function MercuryRecipientManager({
  ...
  async function handleCreate(): Promise<void> {
    if (!adapter.createRecipient) {
      const message = 'Recipient creation is unavailable because no createRecipient adapter was provided.';
      onError?.(message);
      setStatus({ message, tone: 'error' });
      return;
    }
    ...
    setIsSaving(true);
    try {
      const recipient = await adapter.createRecipient(payload);
      setStatus({
  ...
  async function handleUpdate(): Promise<void> {
    if (!adapter.updateRecipient) {
      const message = 'Recipient updates are unavailable because no updateRecipient adapter was provided.';
      onError?.(message);
      setStatus({ message, tone: 'error' });
      return;
    }
    ...
    setIsSaving(true);
    try {
      const recipient = await adapter.updateRecipient(selectedRecipientId, payload);
      setStatus({
  ...
  return (
    <MercuryCard
      ...
      subtitle="Paste a valid Mercury recipient JSON payload to create or patch a recipient without leaving your app."
    >
      <View
        style={{
          gap: 8,
      ...
      >
        <Text style={{ color: '#6b4e00', fontWeight: '700' }}>Recipient JSON guardrails</Text>
        <Text style={{ color: '#6b4e00', fontSize: 12, lineHeight: 18 }}>
          Mercury recipients have strict schemas. Start from the editable JSON loader, then trim the payload down to only the fields you actually want to create or patch.
        </Text>
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        <Pressable onPress={() => setPayloadText('{\n  \n}')} style={secondaryButtonStyle}>
      ...
      <View style={{ gap: 8 }}>
        ...
        <Text style={{ color: mercuryUiTheme.colors.mutedText, fontSize: 12, lineHeight: 18 }}>
          Tip: the loader strips obvious read-only fields, but Mercury may still reject fields it does not allow for updates. If that happens, keep only the patch you actually want to send.
        </Text>
      </View>

      <View style={{ flexDirection: 'row', gap: 8 }}>
        <Pressable onPress={handleCreate} disabled={busy || isSaving} style={primaryButtonStyle}>
          <Text style={primaryButtonTextStyle}>{isSaving ? 'Saving...' : 'Create recipient'}</Text>
        </Pressable>
        <Pressable
